...
/**
 * The logger to write with. Inside a request this is the request-scoped child
 * carrying `requestId`, so a line logged anywhere in the call stack is traceable to
 * the request that caused it without threading a logger argument through every
 * function. Outside a request it is the root logger.
 */
