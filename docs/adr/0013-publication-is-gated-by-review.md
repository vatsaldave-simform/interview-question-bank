# Publication is gated by Reviewer approval
...
are ordered. **Visibility remains the outer gate**, evaluated first on every path through the same
scoped query builder (ADR-0002, ADR-0003); publication state is an inner one applied after it. A
...
The gate is on entry to the bank and nowhere else: edits to a Published Question go live. Sending
...
Every indistinguishability test now has two axes to hold across rather than one, and they have to
hold in combination: not Visible, not Published, and both at once.
